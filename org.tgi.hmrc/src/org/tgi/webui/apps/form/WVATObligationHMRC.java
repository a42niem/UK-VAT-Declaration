...
//				throw new WrongValueException(tbPeriodKey, "Period key must be set !");
			...
			if (fieldDateAcct.getValue() != null) {
				...
				fromDate = String.valueOf(fromCal.get(Calendar.YEAR)) + "-" 
				           + String.valueOf(fromCal.get(Calendar.MONTH)) + "-"
				           + String.valueOf(fromCal.get(Calendar.DAY_OF_MONTH));
			}
			
			String toDate = null;
			if (fieldDateAcct2.getValue() != null) {
				Calendar toCal = TimeUtil.getCalendar((Timestamp) fieldDateAcct2.getValue());
				fromDate = String.valueOf(toCal.get(Calendar.YEAR)) + "-" 
				           + String.valueOf(toCal.get(Calendar.MONTH)) + "-"
				           + String.valueOf(toCal.get(Calendar.DAY_OF_MONTH));
			}
			...
			if(tbStatus.getValue() != "O" && fieldDateAcct.getValue()==null)
				throw new WrongValueException(fieldDateAcct.getComponent(), "StartDate must be set !");
			if(tbStatus.getValue() != "O" && fieldDateAcct2.getValue()==null)
